...
class QueryBuilder {
  ...
  execute(): string {
    ...
    return `%cselect ${fields} from ${this.table} ${conditions} ${orderFields} ${limitCount}`;
  }
}
...
function main() {
  ...
  console.log(usersQuery, COLORS.orange);

  const usersQuery2 = new QueryBuilder("users")
    .select("id", "name", "email")
    .where("age > 18")
    .where("country = 'Cri'")
    .where("country = 'Sochimilca'")
    .orderBy("name", "ASC")
    .limit(10)
    .execute();

  console.log("%cConsulta:\n", COLORS.red);
  console.log(usersQuery2, COLORS.orange);
}
